"""
Parallel Experiment Runner for Green Object Generalization (WITH AUTOMATIC RETRY)

Trains all algorithms (SR, DQN, LSTM, WVF) across multiple seeds with unseen green object evaluation
using SLURM cluster parallelization.

This version automatically retries failed jobs (e.g., due to node graphics issues).

Usage:
    python run_parallel_experiments_green.py
...
BASE_OUTPUT_DIR = Path("experiment_results_green")
TIMESTAMP = datetime.now().strftime("%Y%m%d_%H%M%S")
EXPERIMENT_DIR = BASE_OUTPUT_DIR / f"green_comparison_{TIMESTAMP}"
...
def run_single_experiment(algorithm: str, seed: int, config: dict, output_dir: Path):
    ...
    from experiment_utils_green import (
        train_sr_agent,
        train_unified_dqn,
    ...
    print(f"\n{'='*70}")
    print(f"STARTING GREEN EXPERIMENT JOB: Algorithm={algorithm}, Seed={seed}")
    ...
    with open(run_dir / "config.json", 'w') as f:
        json.dump({
            ...
            "experiment_type": "green_object_generalization"
        }, f, indent=2)
    ...
    try:
        ...
        print(f"\n{'='*70}")
        print(f"GREEN EXPERIMENT JOB COMPLETE: Algorithm={algorithm}, Seed={seed}")
        ...
        print(f"\n{'='*70}")
        print(f"GREEN EXPERIMENT JOB FAILED: Algorithm={algorithm}, Seed={seed}")
        print(f"Error: {e}")
        print(f"{'='*70}\n")
...
def main():
    ...
    print(f"\n{'='*80}")
    print(f"PARALLEL GREEN OBJECT EXPERIMENT LAUNCHER (WITH AUTOMATIC RETRY)")
    print(f"{'='*80}")
    ...
    print(f"\nExperiment type: Zero-shot green object generalization")
    print(f"Training: red/blue objects only")
    print(f"Evaluation: primitives (seen) + compositional (seen) + GREEN (unseen)")
    print(f"{'='*80}\n")
    ...
    metadata = {
        ...
        "seen_eval_tasks": ["red", "blue", "box", "sphere", "red_box", "red_sphere", "blue_box", "blue_sphere"],
        "unseen_eval_tasks": ["green", "green_box", "green_sphere"]
    }
    ...
    try:
        ...
        print(f"\n{'='*80}")
        print(f"ALL GREEN EXPERIMENT JOBS COMPLETED!")
        print(f"{'='*80}")
        ...
        if successful > 0:
            ...
            try:
                from aggregate_and_plot_green import aggregate_and_plot_green
                aggregate_and_plot_green(EXPERIMENT_DIR)
            except ImportError:
                print("aggregate_and_plot_green.py not found. Run it manually:")
                print(f"  python aggregate_and_plot_green.py {EXPERIMENT_DIR}")
