"""
Experiment Utilities for Green Object Generalization

Training and evaluation functions for all algorithms with unseen green object evaluation.
All algorithms trained uniformly (random primitive tasks: red, blue, box, sphere - NO GREEN)
and evaluated on:
1. Primitive tasks (seen)
2. Compositional tasks (seen: red/blue + shape)
3. GREEN tasks (unseen: green, green_box, green_sphere)

This tests zero-shot generalization to novel color features.
"""
...
# TRAINING TASKS - Only red/blue objects (NO GREEN!)
PRIMITIVE_TASKS = [
...
# SEEN COMPOSITIONAL TASKS (red/blue + shape)
SEEN_COMPOSITIONAL_TASKS = [
...
# UNSEEN GREEN TASKS (zero-shot generalization)
GREEN_SIMPLE_TASKS = [
    {"name": "green", "features": ["green"], "type": "green_simple"},
]

GREEN_COMPOSITIONAL_TASKS = [
    {"name": "green_box", "features": ["green", "box"], "type": "green_compositional"},
    {"name": "green_sphere", "features": ["green", "sphere"], "type": "green_compositional"},
]
...
def detect_cube(model, obs, device, transform, pos_mean=0.0, pos_std=1.0):
    ...
    with torch.no_grad():
        ...
        # 6 objects now: red_box, blue_box, green_box, red_sphere, blue_sphere, green_sphere
...
def train_sr_agent(seed, training_episodes, eval_episodes_per_task, max_steps, env_size,
    ...
    """Train Successor Representation agent with green object evaluation."""
    ...
    print(f"Training tasks: {[t['name'] for t in PRIMITIVE_TASKS]} (NO GREEN)")
    print(f"{'='*70}\n")
    ...
    # Create environment in TRAINING mode (no green objects)
    ...
    print("Starting training phase (red/blue only)...")
    ...
    # PRIMITIVE EVALUATION (SEEN - still in training mode)
    print(f"\nStarting PRIMITIVE evaluation phase (seen, red/blue only)...")
    env.set_training_mode(True)  # Keep training mode
    ...
    # SEEN COMPOSITIONAL EVALUATION (red/blue + shape, still training mode)
    print(f"\nStarting SEEN COMPOSITIONAL evaluation phase...")
    seen_comp_eval_labels = []
    ...
    for comp_task in SEEN_COMPOSITIONAL_TASKS:
        ...
        for ep in range(eval_episodes_per_task):
            ...
            all_rewards.append(episode_reward)
            seen_comp_eval_labels.append(f"eval_seen_comp_{task_name}")
    
    # GREEN SIMPLE EVALUATION (UNSEEN - switch to eval mode!)
    print(f"\nStarting GREEN SIMPLE evaluation phase (UNSEEN - zero-shot)...")
    print("Switching environment to evaluation mode (spawning green objects)...")
    env.set_training_mode(False)  # Enable green objects
    green_simple_eval_labels = []
    ...
    for green_task in GREEN_SIMPLE_TASKS:
        ...
        for ep in range(eval_episodes_per_task):
            ...
            for step in range(max_steps):
                detection_result = detect_cube(cube_model, obs, device, transform, pos_mean, pos_std)
                agent.update_feature_map(detection_result['detected_objects'], detection_result['positions'])
                agent.compose_reward_map(green_task)
                agent.compute_wvf()
                
                action = agent.sample_action_with_wvf(obs, epsilon=0.0)
                obs, env_reward, terminated, truncated, info = env.step(action)
                
                if check_task_satisfaction(info, green_task):
                    episode_reward += env_reward
            ...
            all_rewards.append(episode_reward)
            green_simple_eval_labels.append(f"eval_green_simple_{task_name}")
    
    # GREEN COMPOSITIONAL EVALUATION (UNSEEN)
    print(f"\nStarting GREEN COMPOSITIONAL evaluation phase (UNSEEN)...")
    green_comp_eval_labels = []
    ...
    for green_comp_task in GREEN_COMPOSITIONAL_TASKS:
        ...
        for ep in range(eval_episodes_per_task):
            ...
            for step in range(max_steps):
                detection_result = detect_cube(cube_model, obs, device, transform, pos_mean, pos_std)
                agent.update_feature_map(detection_result['detected_objects'], detection_result['positions'])
                agent.compose_reward_map(green_comp_task)
                agent.compute_wvf()
                
                action = agent.sample_action_with_wvf(obs, epsilon=0.0)
                obs, env_reward, terminated, truncated, info = env.step(action)
                
                if check_task_satisfaction(info, green_comp_task):
                    episode_reward += env_reward
            ...
            all_rewards.append(episode_reward)
            green_comp_eval_labels.append(f"eval_green_comp_{task_name}")
    
    all_labels = (episode_labels + primitive_eval_labels + seen_comp_eval_labels + 
                  green_simple_eval_labels + green_comp_eval_labels)
    ...
    return {
        ...
        "seen_comp_eval_episodes": len(seen_comp_eval_labels),
        "green_simple_eval_episodes": len(green_simple_eval_labels),
        "green_comp_eval_episodes": len(green_comp_eval_labels),
...
def train_unified_dqn(seed, training_episodes, eval_episodes_per_task, max_steps, env_size,
    ...
    """Train Unified DQN agent with green object evaluation."""
    ...
    # Create environment in TRAINING mode (no green)
    env = DiscreteMiniWorldWrapper(size=env_size, render_mode="rgb_array", training_mode=True)
    ...
    # GREEN SIMPLE EVALUATION (UNSEEN)
    print("\nStarting GREEN SIMPLE evaluation phase (UNSEEN)...")
    env.set_training_mode(False)  # Enable green objects
    green_simple_eval_labels = []
    ...
    for green_task in GREEN_SIMPLE_TASKS:
        ...
        for ep in range(eval_episodes_per_task):
            ...
            for step in range(max_steps):
                action = agent.select_action(obs, green_task['features'], epsilon=0.0)
                obs, _, terminated, truncated, info = env.step(action)
    ...
    for green_comp_task in GREEN_COMPOSITIONAL_TASKS:
        ...
        for ep in range(eval_episodes_per_task):
            ...
            for step in range(max_steps):
                action = agent.select_action(obs, green_comp_task['features'], epsilon=0.0)
                obs, _, terminated, truncated, info = env.step(action)
...
def train_unified_lstm_dqn(seed, training_episodes, eval_episodes_per_task, max_steps, env_size,
                           learning_rate, gamma, epsilon_decay, output_dir):
    """Train Unified LSTM-DQN agent with green object evaluation."""
    ...
    # Create environment in TRAINING mode
    env = DiscreteMiniWorldWrapper(size=env_size, render_mode="rgb_array", training_mode=True)
...
def train_unified_wvf(seed, training_episodes, eval_episodes_per_task, max_steps, env_size,
                      learning_rate, gamma, epsilon_decay, output_dir):
    """Train Unified WVF agent with green object evaluation."""
    ...
    for green_comp_task in GREEN_COMPOSITIONAL_TASKS:
        env.set_task(green_comp_task)
        task_name = green_comp_task['name']
        features = green_comp_task['features']
        print(f"Evaluating green compositional task: {task_name} (UNSEEN)...")
