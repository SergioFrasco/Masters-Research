...
class ExperimentRunner:
    ...
    def run_successor_experiment(self, episodes=1, max_steps=200, seed=20):
        ...
        for episode in tqdm(range(episodes), desc=f"Masters Successor (seed {seed})"):
            ...
            for step in range(max_steps):
                ...
                input_partial = normalized_partial[np.newaxis, ..., np.newaxis]  # (1, H, W, 1)
                ...
                # Update only visible, unvisited positions with AE predictions
                for y in range(agent.true_reward_map.shape[0]):
                    for x in range(agent.true_reward_map.shape[1]):
                        # Check if position is unvisited AND within agent's forward-facing 7x7 view
                        if (not agent.visited_positions[y, x] and is_within_forward_view(agent_position[0], agent_position[1], agent_dir, y, x, predicted_partial_2d.shape)):
                            
                            predicted_value = predicted_partial_2d[y, x]
                ...
                center_x = 3
                center_y = 7 - 1  # Agent is at bottom center of the 7x7 view

                if (abs(predicted_partial_2d[center_y, center_x]- agent.true_reward_map[agent_position[0], agent_position[1]])> train_vision_threshold):
                    trigger_ae_training = True

                if trigger_ae_training:
                    
                    # Agent is at bottom center (6, 3) of 7x7 view, so we need:
                    # - 6 rows above agent (y-6 to y-1) 
                    # - Current row (y)
                    # - 3 columns left and right of agent (x-3 to x+3)
                    start_y = agent_position[1] - 6
                    end_y = agent_position[1] + 1  # +1 because range is exclusive
                    start_x = agent_position[0] - 3
                    end_x = agent_position[0] + 4  # +4 to get 7 columns total
                    
                    # Create 7x7 target, handling boundary cases
                    target_7x7 = np.zeros((7, 7), dtype=np.float32)
                    
                    # Calculate which part of the true_reward_map we can actually see
                    map_start_y = max(0, start_y)
                    map_end_y = min(agent.true_reward_map.shape[0], end_y)
                    map_start_x = max(0, start_x)
                    map_end_x = min(agent.true_reward_map.shape[1], end_x)
                    
                    # Calculate corresponding indices in the 7x7 target
                    target_start_y = max(0, -start_y)
                    target_end_y = target_start_y + (map_end_y - map_start_y)
                    target_start_x = max(0, -start_x)
                    target_end_x = target_start_x + (map_end_x - map_start_x)
                    
                    # Copy the visible portion
                    if map_end_y > map_start_y and map_end_x > map_start_x:
                        target_7x7[target_start_y:target_end_y, target_start_x:target_end_x] = \
                            agent.true_reward_map[map_start_y:map_end_y, map_start_x:map_end_x]
            ...
            if episode % 100 == 0:
                ...
                # visible_positions = get_visible_global_positions(agent_position, agent_dir, view_size, 10)
                    ...
                    for x in range(env.unwrapped.size):
                        for y in range(env.unwrapped.size):
                            cell = env.unwrapped.grid.get(x, y)
                            # FLIPPED: Use [x, y] for grid array indexing (swapped from [y, x])
                            if cell is None:
                                env_grid[x, y] = 0.0  # Empty space
                            elif cell.type == 'wall':
                                env_grid[x, y] = 0.5  # Wall
                            elif cell.type == 'goal':
                                env_grid[x, y] = 1.0  # Goal/Reward
                            else:
                                env_grid[x, y] = 0.2  # Other objects
                    ...
                    # FLIPPED: When plotting with matplotlib, swap x and y
                    axes[0].plot(agent_y, agent_x, 'ro', markersize=12, markeredgecolor='white', markeredgewidth=2)
                    ...
                    # FLIPPED: Swap dx and dy, and swap agent_x and agent_y
                    axes[0].arrow(agent_y, agent_x, dy, dx, head_width=0.15, head_length=0.1, 
                    ...
                    axes[1].plot(agent_y, agent_x, 'co', markersize=12, markeredgecolor='white', markeredgewidth=2)
                    axes[1].arrow(agent_y, agent_x, dy, dx, head_width=0.15, head_length=0.1, 
...
# HELPER FUNCTIONS
def is_within_forward_view(agent_y, agent_x, agent_dir, target_y, target_x, predicted_map_shape):
    """Check if target is within MiniGrid's 7x7 forward-facing view and within bounds"""
    
    # First check if target is within the predicted map bounds
    if not (0 <= target_y < predicted_map_shape[0] and 0 <= target_x < predicted_map_shape[1]):
        return False
    ...
    if agent_dir == 3:  # Facing North (up)
        return rel_y >= -6 and rel_y <= 0 and abs(rel_x) <= 3
    elif agent_dir == 0:  # Facing East (right)
        return rel_x >= 0 and rel_x <= 6 and abs(rel_y) <= 3
    elif agent_dir == 1:  # Facing South (down)
        return rel_y >= 0 and rel_y <= 6 and abs(rel_x) <= 3
    elif agent_dir == 2:  # Facing West (left)
        return rel_x >= -6 and rel_x <= 0 and abs(rel_y) <= 3
