...
def train_successor_agent(agent, env, episodes=2001, ae_model=None, max_steps=200, epsilon_start=1.0, epsilon_end=0.1, epsilon_decay=0.9995, train_vision_threshold=0.1, device='cpu', optimizer=None, loss_fn=None):
    """
    Training loop with egocentric view for autoencoder - using exact position
    """
    episode_rewards = []
    ...
    for episode in tqdm(range(episodes), "Training Successor Agent"):
        ...
        for step in range(max_steps):
            ...
            # Get exact agent position directly from environment (like reference)
            agent_pos = tuple(env.unwrapped.agent_pos)  # Use exact position
            agent_dir = env.unwrapped.agent_dir  # Use exact direction
            
            obs, reward, done, _, _ = env.step(current_action)
            next_state_idx = agent.get_state_index(obs)

            # Update state occupancy tracking
            ...
            exact_pos = tuple(env.unwrapped.agent_pos)
            exact_dir = env.unwrapped.agent_dir

            # 1. CREATE EGOCENTRIC VIEW INPUT
            egocentric_input = create_egocentric_view(env, exact_pos, exact_dir, VIEW_SIZE, EMPTY_SPACE, REWARD, OUT_OF_BOUNDS, WALL, done=done)
            ...
            for ego_y in range(VIEW_SIZE):
                for ego_x in range(VIEW_SIZE):
                    global_x, global_y = egocentric_to_global_coords(ego_x, ego_y, int(exact_pos[0]), int(exact_pos[1]), exact_dir, VIEW_SIZE)
            ...
            # Mark current position with ground truth (like reference)
            ...
            visible_global_positions = get_visible_global_positions(exact_pos, exact_dir, VIEW_SIZE, env.unwrapped.size)
            ...
            egocentric_target = create_egocentric_target_from_true_map(agent.true_reward_map, exact_pos, exact_dir, done, VIEW_SIZE, OUT_OF_BOUNDS)
            ...
            # 6. UPDATE WVF (same as before but simplified like reference)
        ...
        if episode % 50 == 0:
            ...
            save_egocentric_view_visualization(
                ...
                episode,
                step,
                agent_pos=exact_pos,  # Use exact position
                agent_dir=exact_dir,  # Use exact direction
...
def main():
    ...
    if rewards and len(rewards) > 0:  # Check if rewards list is not empty
        # Convert to pandas Series for rolling average
        rewards_series = pd.Series(rewards)
        rolling_window = min(20, len(rewards))  # Use smaller window if not enough data
        ...
        # Plot raw rewards
        plt.plot(range(len(rewards)), rewards, label='Episode Reward', alpha=0.5)
        
        # Plot smoothed rewards (skip NaN values at the beginning)
        valid_indices = ~smoothed_rewards.isna()
        if valid_indices.any():  # Only plot if we have valid smoothed data
            plt.plot(np.arange(len(smoothed_rewards))[valid_indices], 
                    smoothed_rewards[valid_indices], 
                    label=f'Rolling Avg (window={rolling_window})', linewidth=2)
        ...
        plt.close()  # Add this to clean up
    else:
