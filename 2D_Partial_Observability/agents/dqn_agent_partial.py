...
class DQNAgentPartial:
    ...
    def __init__(self, env, learning_rate=0.001, gamma=0.99, epsilon_start=1.0, 
        ...
        # State representation: 7x7 view (49) + position (2) + direction (4) + reward context (optional)
        self.state_dim = 7 * 7 + 2 + 4  # 55 features
    ...
    def get_dqn_state(self, obs):
        ...
        # Extract 7x7 view and flatten
        if 'image' in obs:
            view = obs['image'][0] if len(obs['image'].shape) == 3 else obs['image']
        else:
            # Fallback to zeros if no image
            view = np.zeros((7, 7))
        
        view_flat = view.flatten().astype(np.float32)  # 49 values
        
        # Normalize view values to [0, 1] range
        view_flat = view_flat / 1.0  # Assuming max object type is ~1
        
        # Get normalized position (using path integration)
        if self.internal_pos is not None:
            pos_x = self.internal_pos[0] / (self.grid_size - 1)
            pos_y = self.internal_pos[1] / (self.grid_size - 1)
        else:
            pos_x, pos_y = 0.0, 0.0
        ...
        if self.internal_dir is not None:
            direction_onehot[self.internal_dir] = 1.0
