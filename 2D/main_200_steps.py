...
class ExperimentRunner:
    ...
    def run_successor_experiment(self, episodes=5000, max_steps=200, seed=20):
        ...
        for episode in tqdm(range(episodes), desc=f"Masters Successor (seed {seed})"):
            ...
            # Track if we're in random exploration mode after reaching goal
            random_steps_remaining = 0
            goals_reached = 0  # Track number of goals reached in this episode
            ...
            for step in range(max_steps):
                ...
                if done and step < max_steps - 1:  # Goal reached but not at max steps
                    goals_reached += 1
                    random_steps_remaining = 10  # Start 10 random steps
                    # Reset the environment to continue playing
                    obs = env.reset()
                    next_state_idx = agent.get_state_index(obs)
                    done = False  # Override done flag to continue
                ...
                # Choose next action based on current mode
                if random_steps_remaining > 0:
                    # In random exploration mode after reaching goal
                    next_action = agent.sample_random_action(obs, epsilon=1.0)  # Full random
                ...
                # Learning Signal
                # Only update if we actually got a reward (not just because we're continuing after goal)
                if reward > 0:  # Got actual reward
            ...
            if episode % 100 == 0:
                ...
                # Log goals reached in this episode
                # print(f"Episode {episode}: Goals reached = {goals_reached}")
