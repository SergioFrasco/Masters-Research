...
class CubeDataset(Dataset):
    def __init__(self, csv_path, img_root, split="train", transform=None):
        ...
        # 4 independent binary labels
        self.label_cols = ["red_box", "blue_box", "red_sphere", "blue_sphere"]
    ...
    def __getitem__(self, idx):
        ...
        pos_normalized = np.zeros(8, dtype=np.float32)
        for i in range(8):
...
class CubeDetector(nn.Module):
    def __init__(self, num_labels=4):
        ...
        # Multi-label classification head (4 independent binary predictions)
        self.classifier = nn.Linear(512, num_labels)
        
        # Regression head for 8 position values (4 objects × 2 coordinates each)
        self.regressor = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(128, 8)  # red_box_dx/dz, blue_box_dx/dz, red_sphere_dx/dz, blue_sphere_dx/dz
        )

    def forward(self, x):
        feats = self.features(x).squeeze(-1).squeeze(-1)
        cls_logits = self.classifier(feats)  # [batch, 4]
        pos_preds = self.regressor(feats)     # [batch, 8]
...
def masked_mse(pred, target, labels):
    ...
    pred: [batch, 8] - predicted positions
    target: [batch, 8] - target positions
    labels: [batch, 4] - binary labels for [red_box, blue_box, red_sphere, blue_sphere]
    """
    # Create masks for each object type
    mask_red_box = labels[:, 0]      # [batch]
    mask_blue_box = labels[:, 1]     # [batch]
    mask_red_sphere = labels[:, 2]   # [batch]
    mask_blue_sphere = labels[:, 3]  # [batch]
    ...
    loss_red_sphere = ((pred[:, 4:6] - target[:, 4:6]) ** 2).mean(dim=1)    # [batch]
    loss_blue_sphere = ((pred[:, 6:8] - target[:, 6:8]) ** 2).mean(dim=1)   # [batch]
    ...
    total_valid = mask_red_box.sum() + mask_blue_box.sum() + mask_red_sphere.sum() + mask_blue_sphere.sum()
    
    if total_valid == 0:
        return torch.tensor(0.0, device=pred.device, requires_grad=True)
    
    # Average over all valid object predictions
    total_loss = (loss_red_box.sum() + loss_blue_box.sum() + 
                  loss_red_sphere.sum() + loss_blue_sphere.sum()) / total_valid
...
def load_model_and_predict(model_path, image_path, threshold=0.5):
    ...
    with torch.no_grad():
        ...
        positions = {}
        pos_labels = ["red_box", "blue_box", "red_sphere", "blue_sphere"]
